            ...
            <div class="menu">
                ...
                <div>
                    ...
                    <div class="links sales-links">
                        ...
                        <div class="tabs sales-tabs sales-selected-tab"><span class="fa tab-icon fa-calendar-day"></span><span class="tab-name">Daily</span></div>
...
<%
    try{  
            ...
            //Products today
            String TodayQuery = "SELECT * FROM "+salesTable+" WHERE DateSold = ?";
            String TodayValuesQuery = "SELECT COUNT(*) AS Today, SUM(TotalAmount) AS TotalSales FROM "+salesTable+" WHERE DateSold = ?";
            PreparedStatement todaySales =  con.prepareStatement(TodayQuery);
            PreparedStatement todaySalesValues =  con.prepareStatement(TodayValuesQuery);
            todaySales.setString(1,today);
            todaySalesValues.setString(1,today);
            ResultSet todayValuesResults = todaySalesValues.executeQuery(); 
            if (todayValuesResults.next()){

%>   
                    <div class="default-card-header">Today's Sales</div>
                    <div class="shop-sales-area">
                        <div class="detail detail-shop-sales">
                            <div class="detail-box"> 
                                ...
                                <div class="number-output"><%= todayValuesResults.getInt(1)%></div> 
                            </div>
                        </div>
                        ...
                        <div class="detail detail-shop-sales">
                            <div class="detail-box "> 
                                ...
                                <div class="number-output"><%= todayValuesResults.getInt(2)%>/-</div> 
                            </div>
                        </div>
                    </div>
<%
...
<%
                ...
                ResultSet todayResults = todaySales.executeQuery(); 
                while(todayResults.next()){
%>
                            <tr>
                                <td class="table-image"><img width="50px" height="50px"  src="uploads/<%=todayResults.getString(5) %>"/></td>
                                <td><%=todayResults.getString("BagID")%></td>
                                <td><%=todayResults.getString("Name")%></td>
                                <td><%=todayResults.getString("Description")%></td>
                                <td><%=todayResults.getString("QuantitySold")%></td>
                                <td><%=todayResults.getString("DateSold")%></td>
                                <td><%=todayResults.getString("Price")%></td>
                                <td><%=todayResults.getString("TotalAmount")%></td>
                            </tr>
<%
        }                            
        con.close();
